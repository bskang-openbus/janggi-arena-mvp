...
### [2026-08-03 08:1x] P7 컴퓨터 대국 추가 (사용자 요청)
- 배경: 사용자가 로컬 대국이 핫시트임을 확인 후 AI 상대(난이도 3단계)를 요청. PRD의 "차기 버전" 항목을 앞당겨 P7로 편성
- 결정: AI 계약을 docs/AI_API.md로 확정 (engine에 chooseAiAction, 3난이도, 웹은 Web Worker 소비). 기물 점수는 대한장기협회 통용치(차13 포7 마5 상3 사3 졸병2) 기반
- 사유: 엔진(순수 TS·의존성 0)에 두면 서버 AI 대국 확장도 가능. 워커 격리로 60fps 유지
- 영향: TASKS.md P7 신설, packages/engine AI 모듈, apps/web 모드 추가

### [2026-08-03 11:10] P7-ai 엔진 AI 탐색 모듈 — 탐색·평가·난이도 캘리브레이션 (AI 담당 에이전트)

- 배경: docs/AI_API.md 계약(`chooseAiAction(state, opts): AiResult`)을 packages/engine에 구현. 룰 엔진(215 green)·apps/web 무수정, 런타임 의존성 0 유지
- 결정:
  1. **루트는 엔진, 내부는 자체 보드 탐색** — 후보 수는 반드시 엔진 `allLegalActions(state)`에서 받고(반환 전 `isLegal` 재검증) 트리 내부만 `cloneBoard` + make/unmake로 돈다. 사유: `applyAction`은 노드마다 보드 복제 + `positionCounts` Map 복사 + `positionKey`(90자 문자열) + 전수 의사합법 수 생성을 하므로 측정상 ~1만 노드/s가 한계였다. 이 구조로 **9.5만 노드/s**를 얻었고, 그래도 "AI가 비합법 수를 반환할" 위험은 루트가 엔진이라 구조적으로 0이다
  2. **역방향 공격 판정 `ai/attacks.ts` 신설** — 엔진의 `isCheck`는 상대 의사합법 수를 전부 생성하지만(O(전체 수)), 탐색은 후보 수마다 자살수 검사를 해야 해서 이게 병목이다. 목표 칸에서 거꾸로 훑는 O(1)급 판정을 movegen.ts에서 **파생**시켰다(차/포 4방향 + 궁성 대각, 마 8방위 + 멱, 상 8방위 + 멱2, 졸 전진·좌우 + 궁성 대각 전진, 궁/사 궁성 선). 규칙 이중 구현의 위험은 **등가성 퍼즈로 상환**: 무작위 진행 60국면 × 양측 × 전 칸(8,745회 비교, 자기 기물·포가 놓인 칸 제외) + 400국면의 궁 기준 `isCheck` 일치 — 불일치 0. 포가 놓인 칸을 제외하는 이유는 "포는 포를 잡지 못한다"가 *잡히는 쪽* 제약이라 장군 판정(목표=궁)에는 무관하기 때문
  3. **평가**: 물량(차1300 포700 마500 상300 사300 졸병200, 궁 0 — 승패로만)에 위치 가중을 물량보다 훨씬 작게 얹었다(졸 1칸 전진 +8, 중앙 3파일 전진졸 +6, 마·상 진영선 이탈 +8, 궁 인접 사 +10, 차 직선 활로 1칸 +4). 단위는 센티포인트(100 = 1기물점). 초기 국면 평가 = 정확히 0(완전 대칭 검증). 기동력을 차로만 한정한 이유는 리프마다 포·마 이동생성을 돌리면 노드 수가 절반으로 떨어져서다
  4. **장기 특수 규칙**: `pass`도 후보에 넣되 페널티(초급 300 / 중급 60 / 고급 40 센티포인트)를 줘 수 낭비를 막는다. 장군 상태 pass 불가는 엔진 규칙을 그대로 신뢰(생성 단계에서 궁 피격 여부로 판단). **빅장·3회 반복은 무승부**이므로 판정 순서까지 엔진과 동일하게(외통 → 빅장 → 반복) 맞췄다
  5. **무승부 기피치(contempt) 도입** — 무승부를 0으로 두면 *조금* 열세인 쪽이 8~9수째에 궁을 옮겨 빅장 무승부로 도망친다(자가 대국에서 실제 관측). 무승부를 **루트 진영 관점 -contempt**로 평가(부호를 루트 진영에 고정해야 네가맥스 반전에도 일관)하여 초급 900 / 중급 300 / 고급 150을 줬다. 확실히 열세면 여전히 빅장을 노린다 — 룰상 정당한 자원이므로 막지 않는다
  6. **난이도 설계**: ① 초급 = 깊이 1 + 상위 6후보 softmax(T=110). 되받아치기를 아예 보지 못해 스스로 기물을 흘리는 것이 약함의 본질이고, 온도는 "가끔 공짜 졸도 안 잡는" 흔들림을 준다 ② 중급 = 알파베타 깊이 3 고정, 포획 우선 정렬, 정적 탐색 없음(지평선 효과가 남아 사람과 접전), 근소 동점 25센티포인트 무작위 ③ 고급 = 반복 심화 + 트랜스포지션 테이블(zobrist 53비트 키) + 포획 정적 탐색(장군이면 회피 수 전탐색) + 포획>장군>history 정렬, 예산 1200ms
  7. **seed 지정 시 시간 예산을 노드 예산으로 환산**(90 노드/ms) — 벽시계 기반 중단은 기기 속도에 따라 결과가 달라져 "완전 재현"(E2E 요구)과 양립하지 않는다. seed가 있으면 `Date.now()`를 아예 읽지 않고 노드 수로만 중단하므로 같은 seed = 같은 액션·점수·깊이·**노드 수**까지 동일. 무시드는 벽시계(고급 1200ms) 기준이며 새 반복 심화는 예산 45% 이전에만 시작해 1.5배 마진을 지킨다
  8. **RNG 시드 스크램블** — 순수 LCG는 seed 0,1,2…의 첫 출력이 거의 같아(0.2360, 0.2364…) "seed를 순번으로 주는" E2E에서 항상 같은 수가 나왔다. seed를 xorshift-multiply로 섞은 뒤 LCG를 돌린다. `Math.random` 직접 사용은 없다(무시드일 때만 `Date.now()`로 시드 생성)
  9. **종료 국면 처리**: `state.result != null`(또는 합법 액션 0)이면 `EngineError`. 가짜 액션을 만들어 반환하는 것보다 웹이 호출 자체를 막는 편이 안전하다
...
### [2026-08-03 12:00] 헌법 개정 — SD 치비 캐릭터 에셋 예외 도입 (사용자 명시적 결정)
- 배경: 사용자가 전투/피격 연출에 SD 치비 캐릭터 컷인을 요청. CLAUDE.md 1절 3항("전 프로시저럴/합성, 외부 에셋 다운로드 금지")과 정면 충돌하는 프로젝트 헌법 수준 결정이라 오케스트레이터가 임의 진행하지 않고 사용자에게 확인 후 승인받음("우선 프로젝트 헌법을 바꿔줘")
- 결정: CLAUDE.md 1절 3항에 예외 조항 추가 + 8절 신설(SD 치비 캐릭터 에셋 정책). 허용 범위는 포획/피격 연출 컷인 스프라이트로 한정, 보드·기물 본체·UI·배경은 여전히 100% 프로시저럴. 생성 도구는 Higgsfield MCP만(character-sheet 워크플로우 + generate_image + remove_background), 인터넷 스크랩/타사 IP 자산은 절대 금지. 저장소에 정적 파일로 커밋, 런타임 외부 호출 없음. 14종(7기물×2진영) 전부가 아닌 진영별 1~2 원형으로 범위 축소
- 사유: 핵심 검증 가치("포획 순간의 전투 연출이 재미를 만드는가")를 실제 캐릭터 아트로 강화하려는 사용자 의도. 다만 보드 렌더링 자체의 결정성·성능·라이선스 리스크를 지키기 위해 적용 범위를 연출 컷인으로 엄격히 한정
- 영향: CLAUDE.md 1절 3항·4절·8절(신설), TASKS.md에 P8로 편성 예정

### [2026-08-03 15:10] P8 치비 캐릭터 14종 생성 완료 (오케스트레이터 직접 수행)
- 배경: 헌법 8절에 따라 SD 치비 캐릭터 생성. 사용자 지시로 8절의 "1~2 원형 축소" 대신 14기물 전원 개별 캐릭터로 확대, 여성 60% 이상, 한자는 머리/등에 크게
- 결정:
  1. 모델: Higgsfield nano_banana_pro (한자 텍스트 렌더링 품질이 선정 기준 — 실제 14종 전부 자형 정확)
  2. 스타일 포뮬러 고정: "cute SD chibi, 2-head-tall, big sparkling eyes, clean bold anime lineart, soft cel shading, Korean traditional fantasy hanbok-style armor" + 초=deep teal/glowing cyan, 한=deep crimson/glowing red-orange
  3. 시안 2종(초 차 머리띠 / 한 포 등깃발) 사용자 피드백 승인 후 12종 병렬 생성. 한자 배치는 기물별 혼용(사용자 선택): 머리띠=사·차(한)·포(초)·마(초)·졸·병, 등깃발(背旗)=궁(어기)·상·포(한)·마(한)·차(초)는 머리띠
  4. 성비: 여성 9(초: 궁·사·차·포·졸 / 한: 사·포·마·병), 남성 5(초: 마·상 / 한: 궁·차·상) = 64%
  5. 원본은 design/chibi/(2:3, 1696x2528 2K)에 커밋. 웹 통합은 배경 제거 후 public/chibi/로 별도 처리
- 사유: 시안 선행 승인으로 12종 재작업 리스크 제거. 원본 보존으로 향후 포즈 확장(공격/피격) 시 스타일 참조 가능
- 영향: design/chibi/*.png 14장 + 컨택트 시트, TASKS.md P8

### [2026-08-03 15:45] P8 치비 캐릭터 포획 컷인 통합 (연출 담당 에이전트)
- 배경: 헌법 8절이 허용한 범위(포획·피격 연출의 컷인 스프라이트)를 실제 연출에 부착. SCENES.md 5절이 그리다 `[S]`로 SKIP됐던 "타격 순간 캐릭터 컷인"을 로우폴리 실루엣 대신 생성 스프라이트로 채운 셈이다
- 결정:
  1. **부착 방식 — 상태머신 무수정**: P6 사운드와 같은 규약. `stage.t`(포획) / `victoryStage.t`(외통)를 rAF로 *관찰*만 하고 좌표·불투명도는 DOM에 직접 기입한다. 스토어에 추가한 것은 `settings.cutin` 한 필드뿐이고 `T`·`VT`·연출 위상 전이는 한 줄도 건드리지 않았다 (프레임당 React state 0회 규약도 그대로 계승)
  2. **타이밍**: `CUT.in = T.impact − 0.28`(0.92s) 슬라이드-인 → `T.impact`(1.2s) 타격 정점 → `CUT.end = T.impact + 0.42`(1.62s) 완전 제거. 연출초 0.70s + 히트스톱 0.10s = **실시간 약 0.80초 노출**. 연출 총 길이(2.9s)를 늘리지 않았으므로 기존 게이트·기보 재생 타이밍에 영향이 없다
  3. **진영 → 화면 가장자리 고정**(초=좌 / 한=우). "공격측은 왼쪽" 같은 역할 기준 대신 진영 기준으로 두면 누가 쳤든 같은 진영은 늘 같은 자리에서 나와 판을 읽는 눈이 흔들리지 않는다
  4. **피격 표현은 15세 수위**: 같은 스프라이트를 CSS 마스크로 재사용한 실루엣 섬광(혈흔 ON=주황·OFF=백금, 상한 0.75) + 흔들림 + 뒤로 밀림 + 페이드아웃. 상한을 0.75로 둔 이유는 그 이상이면 실루엣이 하얗게 날아가 "누가 맞았는지"가 안 읽히기 때문. 고어 표현 없음
  5. **밴드는 캐릭터 박스가 아니라 뷰포트에 앵커**: 1차 구현은 밴드를 캐릭터 폭(≈190px)에 묶어 두어 화면에서 "스티커 한 장"으로 읽혔다. 40vw 기울인 기둥 + 진영색 그라디언트 + 스피드라인(위치가 `stage.t`의 함수라 seek한 프레임이 결정적)으로 교체
  6. **폴백 2중 그물**: 프리로드에서 실패가 확정된 캐릭터는 `<img>` 자체를 붙이지 않고, 붙인 뒤 실패하면 `onError`가 숨긴다. 어느 쪽이든 콘솔 워닝 한 줄 + 기존 파티클/디졸브 연출은 그대로 끝까지 재생 (헌법 8절 "자산 부재가 E2E를 깨뜨리면 안 된다")
  7. **`pointer-events-none`**: 컷인은 연출 오버레이(z-30) 위 z-32에 얹히지만 포인터를 받지 않는다 → "연출 스킵" 탭이 컷인에 가려 먹히지 않는 회귀를 원천 차단(E2E로 고정)
  8. **저사양 모드는 컷인을 끄지 않고 단순 페이드로 축소** — 캐릭터 등장 자체가 이 페이즈의 산출물이라 없애면 저사양 사용자만 기능을 못 본다. transform 애니메이션(슬라이드·스케일·흔들림·스피드라인)만 정지
  9. **기존 p3/p4 스크린샷 테스트는 토글 OFF 없이 그대로 통과**: 픽셀 판정이 `sampleFrame()`(WebGL 캔버스 리드백)이라 DOM 컷인의 영향을 받지 않는다. 오히려 p4-*.png 아침 확인용 샷에 기물별 캐릭터가 함께 담겨 자료 가치가 올라갔다
- 검증: `pnpm -F engine test` **237 green**(회귀 0) · `pnpm -F web build` 성공 · `pnpm -F web e2e` **34 green**(기존 30 회귀 0 + 신규 4) 연속 2회. 스크린샷 자가 평가 1회 이터레이션(밴드 협소→뷰포트 앵커, 피격 섬광 미약→마스크 실루엣 섬광, 캐릭터 과소→공격 52vh·피격 44vh·승리 66vh). artifacts/p8-cutin.png · p8-victory-chibi.png
- 영향: `apps/web/src/components/game/{cutin.ts,CutinLayer.tsx}`(신설), GameScreen(프리로드 + 레이어 마운트), SettingsOverlay(토글 1행), store(`settings.cutin`), E2EBridge(`snapshot().cutin`), `apps/web/e2e/chibi-cutin.spec.ts`(신설), `apps/web/public/chibi/*.webp` 14장 커밋
